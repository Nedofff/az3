...
export default function OneNewsForEdit({id, title, onClick}:{id:string, title:string, onClick:()=>void}) {
  ...
  return (
    <div className='flex'>
        ...
        <button onClick={clickHandler} className='flex border-s-2 text-center items-center text-2xl w-1/6 border-black py-3 rounded-e-lg sm:px-5 sm:pr-6 justify-center duration-500 shadow-lg bg-white hover:bg-red-600 outline-none hover:text-white'><BiSolidTrash/></button>
    </div>
  )
}
