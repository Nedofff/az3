...
export default function OneNewsForEdit({id, title, onClick}:{id:string, title:string, onClick:()=>void}) {
  ...
  return (
    <div className='flex '>
        ...
        <button onClick={clickHandler} className='flex border-s-2 h-full text-2xl w-1/6 border-black py-3 rounded-e-lg px-5 pr-6 justify-center duration-500 shadow-lg bg-white hover:bg-red-600 outline-none hover:text-white'><BiSolidTrash/></button>
    </div>
  )
}
