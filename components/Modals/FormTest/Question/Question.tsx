  ...
  return (
    <>
      ...
      {answers === 'SELECT' ? <Select clickHandler={clickHandler}/> : Object.keys(question.answers).map((keyAnswer: any) => (
          <Answer key={keyAnswer} topId={id} answerRank={keyAnswer} answer={answers[keyAnswer]} clickHandler={clickHandler}/>
      ))}
      </div>
    </>
  );
...
function Answer({answer, topId, clickHandler, answerRank}:{answerRank:string, answer:string, topId:string, clickHandler: (answer:string|null, answerRank:string)=>void}) {
    const id = useId()

    return (
        <label onClick={(e) => clickHandler(e.currentTarget.textContent, answerRank)} className="inline-flex text-start items-baseline" htmlFor={id}>
